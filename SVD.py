...
class download_video():
    ...
    pwsh_run = lambda cmd: sp.run(['powershell','-c', f"{cmd}"], shell=True)
    ...
    def get_formate(self) -> tuple[str, str]:
            ...
            with open("temp/temp.txt","w") as f:
                    sp.run(['pwsh','-c', f"{self.YT_DLP} -F https://player.vimeo.com/video/{self.V_code}"], shell=True, text=True, stdout=f)
    ...
    def create_temp_file_ytdlp(self, type: str) -> None:
        formate: str = {"V" : self.video_form, "A" : self.audio_form}[type]
        down_cmd: str = fr"{self.YT_DLP} -f {formate} 'https://player.vimeo.com/video/{self.V_code}' -o 'temp\temp_{type + "_" + self.V_title}.mp4'"
    ...
    def chick_down_sussed(self) -> bool:
        return path.exists(fr"output\Video_{self.V_title}.mp4")
...
def main() -> None:
    ...
    down_vid.creat_temp_file_ytdlp('A')
    down_vid.creat_temp_file_ytdlp('V')
